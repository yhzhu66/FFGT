...
# from dgl.nn import GraphConv, EdgeConv, GATConv, SAGEConv, GINConv, GATv2Conv, SGConv
from dgl.nn.pytorch import GraphConv, EdgeConv, GATConv, SAGEConv, GINConv, GATv2Conv, SGConv
# from dgl.nn import GraphConv,GATConv, SAGEConv, GINConv, GATv2Conv, SGConv
...
def GNN_layer(GNN, d_in, d_out):
    if GNN == 'GCN':
        ...
        layer = GATv2Conv(d_in, d_out, num_heads=4)
    elif GNN == 'SGC':
        layer = SGConv(d_in, d_out)
...
class SUGRL_Fast(nn.Module):
    def __init__(self, n_in, cfg=None, batch_norm=False, act='gelu', dropout=0.0, final_mlp=0):
        ...
        in_channels = n_in
        for i, v in enumerate(cfg):
            if i ==len(cfg) and self.final_mlp:
                pass
            out_channels = v
...
class GNN_Model(nn.Module):
    def __init__(self, n_in, cfg=None, batch_norm=False, act='relu', gnn='GCN', dropout=0.0, final_mlp=0):
        ...
        # self.dop_layers = nn.Sequential(*dop_layers)
        if self.final_mlp:
            self.mlp = nn.Linear(in_channels, cfg[-1])
        else:
            self.mlp = None
    ...
    def forward(self, A_a, X_a):
        ...
        for i in range(self.layer_num):
            X_a = self.GCN_layers[i](A_a, X_a)
            if self.gnn == "GAT" or self.gnn =="GATv2":
